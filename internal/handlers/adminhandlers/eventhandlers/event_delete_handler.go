...
func (h *eventDeleteHandler) startDelete(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if len(events) == 0 {
		h.messageSenderService.Reply(msg, "No events available for deletion.", nil)
		return handlers.EndConversation()
	}

	title := fmt.Sprintf("Last %d events:", len(events))
	actionDescription := "that you want to delete"
...
func (h *eventDeleteHandler) handleSelectEvent(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if err != nil {
		h.messageSenderService.Reply(msg, "Invalid ID. Please enter a numeric ID or use the cancel button.", nil)
	...
	if !found {
		h.messageSenderService.Reply(
			msg,
			fmt.Sprintf("Event with ID %d not found. Please enter a valid ID or use the cancel button.", eventID),
			nil,
		)
		return nil // Stay in the same state
	}
...
func (h *eventDeleteHandler) handleCallbackConfirmYes(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if err != nil {
		h.messageSenderService.Reply(ctx.EffectiveMessage, "An error occurred while deleting the event.", nil)
	...
	h.messageSenderService.ReplyMarkdown(
		ctx.EffectiveMessage,
		fmt.Sprintf("Event '*%s*' successfully deleted. \n\nTo view all commands, use /%s", eventName, constants.HelpCommand),
		&gotgbot.SendMessageOpts{
			ParseMode: "Markdown",
		},
	)
...
func (h *eventDeleteHandler) handleCancel(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	h.MessageRemoveInlineKeyboard(b, &ctx.EffectiveUser.Id)
	h.messageSenderService.Reply(msg, "Event deletion operation canceled.", nil)
...
func (h *eventDeleteHandler) MessageRemoveInlineKeyboard(b *gotgbot.Bot, userID *int64) {
	...
	if userID != nil {
		messageID, chatID = h.userStore.GetPreviousMessageInfo(
			*userID,
			eventDeleteCtxDataKeyPreviousMessageID,
			eventDeleteCtxDataKeyPreviousChatID,
		)
	}
...
func (h *eventDeleteHandler) SavePreviousMessageInfo(userID int64, sentMsg *gotgbot.Message) {
	h.userStore.SetPreviousMessageInfo(userID, sentMsg.MessageId, sentMsg.Chat.Id,
		eventDeleteCtxDataKeyPreviousMessageID, eventDeleteCtxDataKeyPreviousChatID)
}
